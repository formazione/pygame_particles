...
class Particle:
    ...
    def __init__(self):
        ...
        self.new_surface = pygame.Surface((1400, 900))

        self.frame_rate = pygame.time.Clock().tick
        # loading images for the script
        self.bg = pygame.image.load("catbg2.png").convert_alpha()
    ...
    def generate_one_particle(self, dim):
        direction = [random_pos(), random.randint(1, 5)]
    ...
    def falling_elements(self, elements_list):
        ...
        for particle in elements_list:
            ...
            if particle[0][1] > 750: # when the radius is 0 it is removed, so we do not see it anymore and it's not in the memory
